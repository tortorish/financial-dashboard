...
## Round 6 / Holdings Performance

截图：

- `docs/screenshots/holdings-performance-v1.png`
- `docs/screenshots/holdings-performance-v1-final.png`
- `docs/screenshots/holdings-performance-v1-mobile.png`

Critic 主要意见：

- 新页面虽然有“持仓滞后”提示，但默认按 1 日涨跌排序，并突出“今日较强重仓”，容易让用户把季报前十大当作实时仓位。
- 表格收益列只写 `1日/1月/1年`，普通用户可能误读成基金净值涨跌；需要明确是股票涨跌，且不能用权重简单合成基金收益。
- `存储芯片/CPO/PCB/设备` 等分类对新手仍是行业黑话，需要每类加一句白话解释。
- 大面积红色涨幅和 `hot` 背景会制造追涨焦虑；移动端一次展示 6 个收益字段过密。

Developer 修复：

- 默认排序从 `1日涨跌` 改为 `合计披露权重`，顶部 KPI 从“今日较强重仓”改成“披露权重最高股票”。
- 每只股票行内增加 `行情日期`、`持仓披露日期`、单股数据源和来源基金披露口径。
- 收益列改为 `股票1日/股票1月/股票2月/股票3月/股票半年/股票1年`，并在页面说明“股票涨跌不等于基金净值涨跌，不能用披露权重简单合成基金收益”。
- 每个产业链分类加白话解释，例如设备是卖给晶圆厂的制造/检测机器，PCB 是 AI 服务器高速电路板链。
- 移除高涨幅背景块，移动端只显示 `股票1日/股票3月/股票1年` 三个关键窗口，降低数字噪声。
- 后端把美股 Yahoo 数据改为优先使用调整收盘价，并把 Yahoo 拉取窗口从 1 年改为 2 年，确保 1 年收益有足够历史数据。

验证：

- `npm run build` 通过。
- 重仓股行情刷新成功：唯一重仓股 `63` 只，公开行情覆盖 `62` 只，`1` 只因代码暂无法映射而显示缺失。
- 美股 1 年收益重新可得，例如 `NVDA`、`AVGO`、`TSM` 均返回 1 年字段。
- 桌面浏览器截图确认：默认按披露权重排序，行内显示披露日期和股票数据源。
- 375px 移动检查：`scrollWidth=375`、`clientWidth=375`，二级收益列已隐藏，无横向溢出。
