...
- `docs/screenshots/fund-picker-v1.png`，用于 Critic 审查第一版选基页。

Critic 主要意见：

- 行情源 meta 与实际缓存不一致：页面仍写东方财富/Yahoo，但本次 A股和港股实际大量降级到腾讯 K 线。
- `光通信/设备` 被归到上游，会误导新手；`电力设备/高端制造`、`其它/待分类` 不应进入清晰半导体上中下游选基判断。
- “披露权重”可能被误读成账户仓位或板块真实配置比例；实际是多只基金前十大持仓权重跨基金相加。
- “相对滞涨候选/候选研究”仍容易被金融新手理解为买入信号。

Developer 修复：

- 后端 `holdingPerformance` 增加 `sourceBreakdown`，按实际成功数据源统计；前端显示为“腾讯A股前复权K线 / 腾讯港股前复权K线 / Yahoo美股日线 / 缺失”等中文来源。
- 保留东方财富主源，新增腾讯 A股/港股前复权 K 线 fallback；东方财富空响应或失败时自动降级，不让单一接口波动拖垮页面。
- 调整产业链阶段规则：`CPO/光通信/PCB/终端/服务器链` 优先归下游；`电力设备/高端制造` 和 `其它/待分类` 归入“交叉/待分类”，不参与选基状态判断。
- 将“相对滞涨候选/候选研究”改为“低热度待复核线索/待复核线索”，文案明确不是低估判断，需继续看基金规模、费率、经理、回撤、估值和个人仓位。
- 将“披露权重”改为“样本披露权重合计”，并在卡片底部说明这是跨基金前十大披露权重相加，不代表用户组合仓位，也不能合成基金收益。

验证：

- `npm run build` 通过。
- `node --check server/services/holdingPerformance.js` 和 `node --check server/index.js` 通过。
- 重仓股刷新接口返回：唯一重仓股 `63` 只，本次公开行情覆盖 `61` 只，缺失 `2` 只；实际来源按接口返回写入 `sourceBreakdown`。
- Critic 一轮审查已完成并修复必须项；最终浏览器全页截图因页面过长导致工具超时，改以构建、API 和首版截图审查结果作为本轮验收依据。
