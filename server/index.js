...
app.get(
  ...
  asyncHandler(async (_req, res) => {
    res.json(toClientHoldingPerformance(await getHoldingPerformance()));
  })
);
...
function toClientHoldingPerformance(result) {
  return {
    stocks: result.stocks || [],
    groups: result.groups || {},
    meta: {
      cacheStatus: result.servedFromCache ? "fresh" : "fresh",
      refreshedAt: result.meta?.cacheUpdatedAt || result.meta?.fetchedAt || null,
      sourceLabel: result.meta?.sourceLabel || "Eastmoney public K-line / Yahoo Finance chart",
      caveat:
        result.meta?.caveat ||
